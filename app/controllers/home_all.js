...
function render(e){
	...
	for (var i=0; i < data.length; i++) {
		...
		if(counter % 3 == 0 && counter > 0){
		    ...
		    var container= $.UI.create("View", {classes:['hsize','vert'], backgroundColor: "#ED1C24", width: cell_width, left: 9, top:9,type: 3, record: adsdata[count1]});			
		    var img = $.UI.create("ImageView",{image:adsdata[count1].img_path,width: Ti.UI.FILL,height: Ti.UI.SIZE});			
		    ...
		    var owner_name = (OS_IOS) ? $.UI.create("Label", {classes: ['h6', 'bold','wfill'],color:'#ffffff',  height:14, top:0, textAlign:"left", text: adsdata[count1].merchant_name}) : $.UI.create("Label", {classes: ['h6', 'bold','wfill'],color:'#ffffff',  height:14, top:0, ellipsize: true, wordWrap: false, textAlign:"left", text: adsdata[i1].merchant_name});
	...
	if(pass){
		...
		if(adsdata.length > count1){
			for(var i1=count1;i1<adsdata.length;i1++){
			    ...
			    var img = $.UI.create("ImageView",{image:adsdata[i1].img_path,width: Ti.UI.FILL,height: Ti.UI.SIZE});			
			    ...
			    var view_bottom = $.UI.create("View", {classes:['wfill','hsize','small-padding'], backgroundColor: '#ED1C24'});
			    var view_bottom_right = $.UI.create("View", {classes:['wfill','hsize','vert'], left: 36, backgroundColor: '#ED1C24'});
			    var owner_img = $.UI.create("ImageView", {image:merchantdata.img_path,defaultImage: "/images/logo_small.png", borderRadius: 15, height:30, width: 30, left:0});

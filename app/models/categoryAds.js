		...
		_.extend(Collection.prototype, {
			...
			getLatestAdsByCategory : function(cate_id, start, end, m_id){
				...
				if(typeof(m_id) != "undefined"){
                    ...
                    var sql = "select categoryAds.*, ads.m_id, ads.name, ads.active_date, ads.youtube, ads.category, ads.sales_from, ads.sales_to, ads.expired_date, ads.updated, ads.img_path, ads.img_thumb, ads.a_id, merchants.merchant_name,merchants.img_path as m_img_path from categoryAds left outer join merchants on categoryAds.m_id = merchants.m_id left outer join ads on categoryAds.m_id = ads.m_id where (ads.category like '%," + cate_id + ",%' or ads.category like '%," + cate_id + "' or ads.category like '" + cate_id + ",%' or ads.category = " + cate_id + ") and ads.status = 1 and ads.expired_date >= date('now') and ads.active_date <= date('now') and ads.sales_to >= date('now') order by ads.updated desc limit " + start + ", " + end;
                }

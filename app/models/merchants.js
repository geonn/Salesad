	...
	extendCollection: function(Collection) {
		_.extend(Collection.prototype, {
            ...
            saveMerchants : function(m_id, u_id, parent, name,mobile,area,state_key, state, img_path, longitude, latitude) {
                ...
                if (res.isValidRow()){
             		sql_query = "UPDATE " + collection.config.adapter.collection_name + " SET u_id='"+u_id+"', parent='"+parent+"', name='"+name+"', mobile='"+mobile+"', area='"+area+"', state_key='"+state_key+"', state_name='"+state_name+"', img_path='"+ img_path+"', longitude='"+longitude+"', latitude='"+latitude+"' WHERE m_id='" +m_id+"'";
                }else{
            ...
            saveArray : function(arr){
                ...
                arr.forEach(function(entry) {
					...
					db.execute(sql_query, entry.m_id, entry.u_id, entry.parent,entry.merchant_name,entry.mobile,entry.area,entry.state_key,entry.state_name,entry.longitude,entry.latitude,entry.updated,entry.img_path,entry.is_featured,entry.status);
					...
					db.execute(sql_query, entry.u_id, entry.parent,entry.merchant_name,entry.mobile,entry.area,entry.state_key,entry.state_name,entry.longitude,entry.latitude,entry.updated,entry.img_path,entry.is_featured,entry.status, entry.m_id);
				});
				db.execute("COMMIT");
	            db.close();
	            collection.trigger('sync');

	...
	extendCollection: function(Collection) {
		_.extend(Collection.prototype, {
			...
			ongoingpost: function(e) {
				...
				now.setTime(temp);
				var lastMonth = COMMON.todayDateTime(now);
				var sql = (e.keyword != "") ? "SELECT * FROM " + this.config.adapter.collection_name+" WHERE status = 1 "+sql_uid+" and sales_from >= '"+lastMonth+"' and sales_to >= date('now') and name like '%" + e.keyword + "%' order by `updated` DESC " : (e.categoryname != undefined) ? "SELECT * FROM " + this.config.adapter.collection_name+" WHERE status = 1 "+sql_uid+" and sales_from >= '"+lastMonth+"' and sales_to >= date('now') and category=" + e.categoryname + " order by `updated` DESC " : "SELECT * FROM " + this.config.adapter.collection_name+" WHERE status = 1 "+sql_uid+" and sales_from >= '"+lastMonth+"' and sales_to >= date('now') order by `updated` DESC ";
			...
			expiredpost: function(e) {
				...
				now.setTime(temp);
				var lastMonth = COMMON.todayDateTime(now);
				var sql = (e.keyword != "") ? "SELECT * FROM " + this.config.adapter.collection_name+" WHERE status = 1 "+sql_uid+" and (sales_from + 30) < date('now') and sales_to < date('now') and name like '%" + e.keyword + "%' order by `updated` DESC " : (e.categoryname != undefined) ? "SELECT * FROM " + this.config.adapter.collection_name+" WHERE status = 1 "+sql_uid+" and (sales_from + 30) < date('now') and sales_to < date('now') and category=" + e.categoryname + " order by `updated` DESC " : "SELECT * FROM " + this.config.adapter.collection_name+" WHERE status = 1 "+sql_uid+" and (sales_from + 30) < date('now') and sales_to < date('now') and name like '%" + e.keyword + "%' order by `updated` DESC ";
                db = Ti.Database.open(this.config.adapter.db_name);
